...
CLASS FSReadTXT FROM LONGNAMECLASS
	...
	METHOD _SetOSError()
ENDCLASS
METHOD New( cFName , cFSep , nFBuffer ) CLASS FSReadTXT
	DEFAULT cFSep := CRLF
...
METHOD Open( iFMode ) CLASS FSReadTXT
	DEFAULT iFMode := 0
	::_CleanLastErr()
	If ::nHnd != -1
...
METHOD ReadLine( /*@*/ cReadLine ) CLASS FSReadTXT
	Local cTmp := ''
	Local cBuffer
	Local nRPos
	Local nRead
...
METHOD GetError() CLASS FSReadTXT
Return ::nFerror

METHOD GetOSError() CLASS FSReadTXT
Return ::nOSError

METHOD GetErrorStr() CLASS FSReadTXT
Return ::cFerrorStr

METHOD GetFSize() CLASS FSReadTXT
Return ::nFSize

METHOD _SetError(nCode,cStr) CLASS FSReadTXT
	::nFerror := nCode
	::cFerrorStr := cStr
Return

METHOD _SetOSError(nCode,cStr,nOsError) CLASS FSReadTXT
	::nFerror := nCode
	::cFerrorStr := cStr
	::nOsError := nOsError
Return
METHOD _CleanLastErr() CLASS FSReadTXT
	::nFerror := 0
	::cFerrorStr := ''
	::nOsError := 0
Return
