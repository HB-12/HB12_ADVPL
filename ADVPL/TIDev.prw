...
Static Function TIDevMan()
    ...
    FWMONITORMSG("### TOTVS TI DEVELOPER ###")
...
Static Function Tela(oDlg)
    ...
    FolderB64(oFol:aDialogs[10])        //--Folder Base64 Converter
    FolderSX6(oFol:aDialogs[11])        //--Folder manutenção Grupo
...
Static Function SelEmp(cEmp, cFil, oWnd)
    Local oDlgLogin
    Local oCbxEmp
    Local oFont
    Local cEmpAtu			:= ""
    ...
    DEFINE MSDIALOG oDlgLogin FROM  0,0 TO 210,380  Pixel TITLE "Ambiente "
    ...
    If lOk
        ...
        MsgRun("Montando Ambiente. Empresa [" + cEmp + "] Filial [" + cFil +"].", "Aguarde...", {||lRet := RpcSetEnv(cEmp, cFil,,,,,) } )
        If !lRet
...
Static Function MenuQuery(cCMD, oOwner, np)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstQry[np], cSufix, ""), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function DicBrowse(cAliasDic, oP2, np, oIndice, oMsg, lBut)
    ...
    __aDicBrw[np]:bChange    := {|| (cAliasTst)->(AtuMsg(__aDicBrw[np], oMsg)) }
...
Static Function PrcImpCSV(cArquivo, cAliasTst)
    ...
    If ! oFile:Open()
        MsgAlert("Não foi possivel abrir o arquivo " + cArquivo)
        Return 
    EndIf 
...
Static Function AltReg(oBrowse, cAliasTst, lQuery)
    ...
    If cTipo == "M"
        ...
        DEFINE MSDIALOG oDlg OF oOwner  FROM  0,0 TO 400, 600  Pixel TITLE "Campo memo " + cField + If(! lLeitura, " | Precione CTRL + W para salvar!", "")
            oGet := TMultiGet():New(0,0,bSetGet(&(cMacro)),oDlg,399,049,oFont,.F.,,,,.T.,,,,,, /*lLeitura*/,,,,.F.)
...
Static Function PesqFunc(cChave, aLista)
    ...
    MsgRun("Buscando funções protheus.","Aguarde",{||aFuns := GetFuncArray(cChave, aType, aFile, aLine, aDate, aTime)})
...
Static Function PesqPrg(cChave, aLista)
    ...
    MsgRun("Buscando funções protheus.","Aguarde",{||aFuns := GetFuncArray("*", aType, aFile, aLine, aDate, aTime)})
    For nx := 1 To Len(aFuns)
...
Static Function MenuCmd(cCMD, oOwner)
    ...
    oMenu := tMenu():new(0, 0, 0, 0, .T., , oOwner)
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstCmd, "cmd", ""), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function MenuHtm(cCMD, oOwner)
    ...
    oMenu := tMenu():new(0, 0, 0, 0, .T., , oOwner)
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstHtm, "htm", ""), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function MenuFil(cFiltro, oOwner)
    ...
    oMenu := tMenu():new(0, 0, 0, 0, .T., , oOwner)
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstFil, "filtro", ""), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function FolderExp(oDlg)
    ...
    //oPM:setCSS(cStyle)
        ...
        oPMM:setCSS(cStyle)
            ...
            @ 200, 10 BITMAP oBmp01 NAME "FILTRO"  SIZE 015, 015 OF oPMM PIXEL NOBORDER ON CLICK (MaskDir(),LeDirect(oList01, oGet1, @cPath01), LeDirect(oList02, oGet2, @cPath02))
...
Static Function FolderB64(oDlg)
    ...
    oP1:Align :=CONTROL_ALIGN_TOP
        ...
        oBut  := THButton():New(002, 002,"Converter" ,oDlg,{|| ExecB64(cMemo1, @cMemo2, lMultiline, cOpB64) }, 45, 10, oFontB, "Converte a string") 
...
Static Function ExecB64(cMemo1, cMemo2, lMultiline, cOpB64)
    Local cRetM:= ""
    Local aDataAux := {}
    Local aDataRet := {}
    Local nx  := 0
    ...
    If lMultiline
        ...
        For nx := 1 To Len(aDataAux)
            If cOpB64 == "Encode Base64"
                aAdd(aDataRet,Encode64( aDataAux[nx] ) )
            Else
    ...
    Else
        If cOpB64 == "Encode Base64"
            cRetM += Encode64( cMemo1 )
        Else
            cRetM += Decode64( cMemo1 )
        EndIf
...
Static Function FolderSX6(oFol)
    Local oBut
    LOcal oPS
    Local oPI
    ...
    oPS := TPanelCss():New(,,,oFol)
    oPS :SetCoors(TRect():New( 0,0, 30, 30))
    oPS :Align := CONTROL_ALIGN_TOP

        @ 004, 002 SAY "Grupo:" of oPS SIZE 030, 09 PIXEL
        @ 003, 025 GET cGrupo   of oPS  Pict  "999" SIZE 030, 08 PIXEL Valid If(VldGrupo(cGrupo, @nSize, @nSizeMin, @nSizeMax), PesqSX3(@cGrupo, oLbx, aLista, nSize), .t.)

        @ 004, 062 SAY "Size:" of oPS SIZE 030, 09 PIXEL
        @ 003, 080 GET nSize   of oPS  Pict  "999" SIZE 030, 08 PIXEL Valid If(VldSize(cGrupo, nSize), PesqSX3(@cGrupo, oLbx, aLista, nSize), .t.)

        @ 004, 122 SAY "Size Min:" of oPS SIZE 030, 09 PIXEL
        @ 003, 150 GET nSizeMin    of oPS  Pict  "999" SIZE 030, 08 PIXEL when .F.
    ...
    oPI:Align :=CONTROL_ALIGN_ALLCLIENT
        ...
        oLbx := MsBrGetDBase():New(01,01,490,490,,,, oPI ,,,,,,,,,,,, .F., "", .T.,, .F.,,,)
        oLbx:align := CONTROL_ALIGN_ALLCLIENT
        oLbx:SetArray( aLista )
        oLbx:addColumn( TCColumn():new("X2_ARQUIVO"    , { || oLbx:aArray[oLbx:nAt,  1] },,,, "LEFT" , 100, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("X3_CAMPO"      , { || oLbx:aArray[oLbx:nAt,  2] },,,, "LEFT" , 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("X3_TAMANHO"    , { || oLbx:aArray[oLbx:nAt,  3] },,,, "RIGHT", 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("X3_DECIMAL"    , { || oLbx:aArray[oLbx:nAt,  4] },,,, "RIGHT", 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("X3_GRPSXG"     , { || oLbx:aArray[oLbx:nAt,  5] },,,, "LEFT" , 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("Existe Arq"    , { || oLbx:aArray[oLbx:nAt,  6] },,,, "LEFT" , 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("Qtde Registros", { || oLbx:aArray[oLbx:nAt,  7] },,,, "RIGHT", 060, .F., .F.,,,, .F. ) )
        oLbx:addColumn( TCColumn():new("Analise"       , { || oLbx:aArray[oLbx:nAt,  8] },,,, "LEFT" , 200, .F., .F.,,,, .F. ) )
        oLbx:Refresh()
...
Static Function VldGrupo(cGrupo, nSize, nSizeMin, nSizeMax)
    ...
    If ! SXG->(DbSeek(cGrupo))
        ...
        Return .F. 
    EndIf 
    nSize     := SXG->(FieldGet(FieldPos("XG_SIZE")))
    nSizeMin  := SXG->(FieldGet(FieldPos("XG_SIZEMIN")))
    nSizeMax  := SXG->(FieldGet(FieldPos("XG_SIZEMAX")))
...
Static Function VldSize(cGrupo, nSize)
    ...
    If ! SXG->(DbSeek(cGrupo))
        ...
        Return .F. 
    EndIf 
    If SXG->(FieldGet(FieldPos("XG_SIZE"))) <> nSize
        If ! MsgNoYes("Confirma o ajuste do size do grupo de " + Alltrim(Str(SXG->(FieldGet(FieldPos("XG_SIZE"))))) + " para " + Alltrim(Str(nSize)) )
            Return 
        EndIf 
        SXG->(RecLock("SXG", .F.))
        SXG->(FieldPut(FieldPos("XG_SIZE"), nSize))
        SXG->(MsUnLock())
        Return .t.
    EndIf 
...
Static Function CarregaSX3(cGrupo, nSize)
    ...
    While SX3->(! Eof() )
        ...
        cAnalise := Space(30)
        If nSize <> SX3->(FIELDGET(FIELDPOS("X3_TAMANHO")))
            cAnalise := "DIFERENTE"
        EndIf

        SX2->(DbSeek(SX3->(FieldGet(FieldPos("X3_ARQUIVO")))))

        cArquivo  := Alltrim(SX2->(FieldGet(FieldPos("X2_ARQUIVO"))))
        nTotReg   := 0
        cAlias    := SX3->(FieldGet(FieldPos("X3_ARQUIVO")))
        ...
        aadd(aSX3,  {SX2->(FieldGet(FieldPos("X2_ARQUIVO"))), SX3->(FieldGet(FieldPos("X3_CAMPO"))), SX3->(FIELDGET(FIELDPOS("X3_TAMANHO"))), SX3->(FieldGet(FieldPos("X3_DECIMAL"))), SX3->(FieldGet(FieldPos("X3_GRPSXG"))), cExiste, nTotReg, cAnalise, "", SX3->(Recno()) })
        SX3->(DbSkip())
    End
    SX3->(DbClearFilter())
    SX3->(DbGoto(nRec))
...
Static Function ProcAltSX3(cGrupo, nSize, aSX3, lVarias)
    ...
    For nx:= 1 to len(aSX3)
        ...
        SX3->(DbSeek(cCampo))
        If SX3->(FIELDGET(FIELDPOS("X3_TAMANHO"))) <> nSize
            SX3->(RecLock("SX3", .F.))
            SX3->(FieldPut(FieldPos("X3_TAMANHO"), nSize))
            SX3->(MsUnLock())
            lAtu := .T.
