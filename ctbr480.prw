...
If lOk	
	...
	Else
		CTBR480R3(cItemIni, cItemFim, dDataIni, dDataFim, cMoeda, cSaldo, cBook, cContaIni,; 
				cContaFim, lCusto, cCustoIni, cCustoFim, lCLVL,	cCLVLIni, cCLVLFim,lSalLin,aSelFil)
	EndIf
EndIf
...
oItem := TRSection():New( oReport, STR0028, {"cArqTmp","CT2"},, .F., .F. )	// "Item"
	...
	TRCell():New( oItem, "ITEM"		, "cArqTmp"	, Upper(cSayItem)/*Titulo*/	,/*Picture*/,aTamItem[1]/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
	TRCell():New( oItem, "DESCIT"	, ""       	, STR0039					,/*Picture*/,IIF(lAnalitico,nTamHist+46,39)+aTamConta[1]+nTamCusto+nTamCLVL+TAM_VALOR,/*lPixel*/,/*{|| }*/)	//"DESCRICAO"
	TRCell():New( oItem, "DESANT"	, ""       	, STR0027					,/*Picture*/,Len(STR0027)	,/*lPixel*/,{||STR0027}/*{|| }*/)
	TRCell():New( oItem, "TPSLDANT"	, ""       	, STR0027					,/*Picture*/,TAM_VALOR+2	,/*lPixel*/,/*{|| }*/,/*"RIGHT"*/,,"CENTER")
...
Endif 
	...
	TRCell():New(oLancto, "DOCUMENTO"	,""        , STR0031			,/*Picture*/,22							,/*lPixel*/	,{|| cArqTmp->LOTE+cArqTmp->SUBLOTE+cArqTmp->DOC+cArqTmp->LINHA })// "LOTE/SUB/DOC/LINHA"
	TRCell():New(oLancto, "HISTORICO"	,"cArqTmp" , STR0032			,/*Picture*/,nTamHist+20				,/*lPixel*/	,{|| cArqTmp->HISTORICO })// "Historico"
	...
	TRCell():New(oLancto, "TPSLDATU"	,"cArqTmp" , STR0036			,/*Picture*/,TAM_VALOR+2				,/*lPixel*/	,/*{|| }*/,/*"RIGHT"*/,,"CENTER")// "SALDO ATUAL"
...
oTotais := TRSection():New( oReport,STR0037,,, .F., .F. )		
	TRCell():New(oTotais,"TOT"			,"",STR0039,/*Picture*/,IIF (lAnalitico,nTamTransp, nTamTransp)	,/*lPixel*/,/*{|| code-block de impressao }*/)	//"DESCRICAO"	
	TRCell():New(oTotais,"TOT_DEBITO"	,"",STR0034,/*Picture*/,TAM_VALOR	,/*lPixel*/,/*{|| code-block de impressao }*/,/*"RIGHT"*/,,"CENTER")
	TRCell():New(oTotais,"TOT_CREDITO"	,"",STR0035,/*Picture*/,TAM_VALOR	,/*lPixel*/,/*{|| code-block de impressao }*/,/*"RIGHT"*/,,"CENTER")
	TRCell():New(oTotais,"TOT_ATU"		,"",STR0036,/*Picture*/,TAM_VALOR+2	,/*lPixel*/,/*{|| code-block de impressao }*/,/*"RIGHT"*/,,"CENTER")
...
oCompl := TRSection():New( oReport,STR0038,,, .F., .F. )	//"Complemento"
	TRCell():New(oCompl,"COMP","",Upper(STR0038),/*Picture*/,IIF(lAnalitico,nTamHist+25,28)+aTamConta[1]+nTamCusto+aTamItem[1]+nTamCLVL,/*lPixel*/,/*{|| code-block de impressao }*/)
...
If !(cArqTmp->(RecCount()) == 0 .And. !Empty(aSetOfBook[5]))
	Do While cArqTmp->( ! EoF() ) .And. !oReport:Cancel()
		...
		oItem:Cell("TPSLDANT"):SetBlock( { || ValorCTB(aSaldoAnt[6],,,TAM_VALOR,nDecimais,.T.,cPicture,,,,,,,lPrintZero,.F.) } )
		...
		Do While cArqTmp->(!Eof()) .And. cArqTmp->ITEM == cItemAnt .And. !oReport:Cancel()
			...
			If lAnalitico
				...
				If lTotConta .And. mv_par11 == 1	// Totaliza tb por Conta
					...
					oTotais:Cell("TOT_DEBITO"):SetBlock(	{ || ValorCTB(nTotCtaDeb,,,TAM_VALOR,nDecimais,.F.,cPicture,"1",,,,,,lPrintZero,.F.) } )
					oTotais:Cell("TOT_CREDITO"):SetBlock(	{ || ValorCTB(nTotCtaCrd,,,TAM_VALOR,nDecimais,.F.,cPicture,"2",,,,,,lPrintZero,.F.) } )
		...
		oTotais:Cell("TOT_DEBITO"	):SetBlock(	{ || ValorCTB(nTotDeb  ,,,TAM_VALOR,nDecimais,.F.,cPicture,"1",,,,,,lPrintZero,.F.) } )
		oTotais:Cell("TOT_CREDITO" 	):SetBlock(	{ || ValorCTB(nTotCrd  ,,,TAM_VALOR,nDecimais,.F.,cPicture,"2",,,,,,lPrintZero,.F.) } )
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Function CTBR480R3(cItemIni, cItemFim, dDataIni, dDataFim, cMoeda, cSaldo,;
...
/*/
Function CTR480Imp(lEnd,WnRel,cString,aSetOfBook,lCusto,lCLVL,lAnalitico,Titulo,;
